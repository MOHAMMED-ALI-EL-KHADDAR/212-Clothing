...
function renderCartItems() {
      ...
      <div class="cart-item-details">
        ...
        <div class="cart-quantity-control">
          <button class="qty-btn qty-minus"
            data-product-id="${item.id}" data-color="${item.color}" data-size="${item.size}">−</button>
          <input type="number" class="cart-qty-input"
            data-product-id="${item.id}" data-color="${item.color}" data-size="${item.size}"
  ...
  cartItemsContainer.querySelectorAll('.remove-item-button').forEach(btn => {
    btn.addEventListener('click', e => {
      removeFromCart(e.target.dataset.productId, e.target.dataset.color, e.target.dataset.size);
    });
  });
  ...
  cartItemsContainer.querySelectorAll('.qty-plus').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const item = cart.find(i => i.id === productId && i.color === color && i.size === size);
      if (item) updateCartItemQuantity(productId, color, size, item.quantity + 1);
    });
  });
  ...
  // Typing in the input — use 'input' event (fires immediately, not on blur like 'change')
